...
```
> "C:\PostgreSQL\9.6\bin\raster2pgsql.exe" -I -M -C -s 4326 -t 20x20 C:\tracking_db\data\env_data\raster\srtm_dem.tif env_data.srtm_dem | "C:\PostgreSQL\9.6\bin\psql.exe" -p 5432 -d gps_tracking_db -U postgres -h localhost
```
...
```
> "C:\PostgreSQL\9.6\bin\raster2pgsql.exe" -I -M -C -s 3035 C:\tracking_db\data\env_data\raster\corine06.tif -t 20x20 env_data.corine_land_cover | "C:\PostgreSQL\9.6\bin\psql.exe" -p 5432 -d gps_tracking_db -U postgres -h localhost
```
...
<<<<<<< HEAD
`gdal_translate -of GTIFF "PG:host=eurodeer2.fmach.it dbname=gps_tracking_db user='postgres' schema=env_data table=land_cover_export mode=2" C:\Users\User\Desktop\landcover\land_cover.tif`
=======
```
> gdal_translate -of GTIFF "PG:host=localhost dbname=gps_tracking_db user=postgres schema=env_data table=land_cover_export mode=2" C:\Users\User\Desktop\landcover\land_cover.tif
```
>>>>>>> 218fcba0826506e42601ef81e00e36f3369ce1eb
...
#### Intersect the fixes with the land cover layer for the animal 1
```sql
SELECT  
...
```
#### Calculate the percentage of each land cover class for fixes of the animal 1
```sql
...
#### Intersect the convex hull of animal 1 with the land cover layer
```sql
SELECT 
...
#### Calculate the percentage of each land cover class in the convex hull for the animal 1
```sql
...
#### Calculate the percentage of each land cover class for male/female
```sql
WITH locations_landcover AS
(
SELECT
...
<<<<<<< HEAD
`C:\PostgreSQL\9.6\bin\raster2pgsql.exe -C -r -t 128x128 -F -M -R -N -3000 C:/tracking_db/data/env_data/raster/raster_ts/M*.tif env_data.ndvi_modis | psql.exe -d gps_tracking_db -U postgres -p 5432`
=======
```
raster2pgsql.exe -C -r -t 128x128 -F -M -R -N -3000 C:/tracking_db/data/env_data/raster/raster_ts/MCD*.tif env_data.ndvi_modis | psql.exe -d gps_tracking_db -U postgres -p 5432 -h localhost
```
>>>>>>> 218fcba0826506e42601ef81e00e36f3369ce1eb
...
```
```sql
CREATE INDEX ndvi_modis_reference_date_index
...
```
```sql
CREATE INDEX modis_ndvi_reference_date_index
  ON env_data.modis_ndvi
  USING btree
  (acquisition_date);
```
...
WHERE 
  ...
  ndvi_modis.acquisition_date = pointintime.reference_date;
```
...
FROM  
  main.gps_data_animals,
<<<<<<< HEAD
  env_data.ndvi_modis AS pre,
  env_data.ndvi_modis AS post
=======
...
( 
  ...
  WHERE
    ...
    acquisition_date > '1/1/2007' and acquisition_date < '30/6/2007'
