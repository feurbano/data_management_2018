...
`C:\PostgreSQL\9.6\bin\raster2pgsql.exe -C -r -t 128x128 -F -M -R -N -3000 C:/tracking_db/data/env_data/raster/raster_ts/M*.tif env_data.ndvi_modis | psql.exe -d gps_tracking_db -U postgres -p 5432 -h localhost`
...
#### Extraction of the NDVI values for a set of fixes as temporal interpolation of the 2 closest images for animal 1
```sql
SELECT 
  gps_data_animals_id, 
